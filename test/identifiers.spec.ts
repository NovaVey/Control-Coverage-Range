...
describe("rbaSubject/rbaObject", () => {
  it("matches Principal-Graph's own RBA-exporter convention byte-for-byte: principal ns, `${source}:${scopedExternalId}` id, no sanitization", () => {
    const s = scenario();
    const subject = rbaSubject(s, { source: "manual", externalId: "alice" });
    // Exactly Principal-Graph's own identityRef() convention
    // (principal-graph/src/exporters/rba.ts) — RBA's data-plane id grammar
    // (relationship-based-authorization/src/store/tuples.ts's
    // invalidDataPlaneIdReason()) accepts this real, unescaped id directly.
